...
class SegNet(BaseNetwork):
    def __init__(self, opt):
        super().__init__()
        self.opt = opt
        num_res_blocks = 6
        sp_norm = spectral_norm #nn.BatchNorm2d #norms.get_spectral_norm(opt)
        output_channel = nc = self.opt.label_nc + 1 if self.opt.contain_dontcare_label \
                else self.opt.label_nc#opt.semantic_nc + 1 # for N+1 loss
...
class residual_block_D(nn.Module):
    def __init__(self, fin, fout, opt, up_or_down, first=False):
        ...
        fmiddle = fout
        norm_layer = spectral_norm #nn.BatchNorm2d #norms.get_spectral_norm(opt)
        if first:
