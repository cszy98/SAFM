...
class CityscapesDataset(Pix2pixDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.set_defaults(segepoch=60)
    ...
    def get_paths(self, opt):
        ...
        instance_paths_all = make_dataset(opt.instance_root, recursive=True) 
        if not opt.no_instance:
            instance_paths = [p for p in instance_paths_all if p.endswith('_instanceIds.png')]
        else:
            instance_paths = []
